...
- 닌텐도 사이트 : https://www.nintendo.com/kr/
- 프로젝트명: Nintendo Renewal — IP & Game Experience
- 목적: Nintendo 브랜드 경험을 재해석한 반응형 게임·IP 웹사이트 팀 프로젝트
- 형태: 실제 결제 연동 전 단계의 게임·IP 탐색 및 스토어 프런트엔드 데모
- 핵심 범위: 메인, 마리오·젤다·스플래툰 IP 페이지, 역사, 게임 탐색·상세, E-shop·상품 상세·주문 확인·데모 결제, 커뮤니티, 고객지원, 장바구니, 좋아요, 기존 로그인·회원가입·마이페이지
...
- Pretendard Variable (화면별 디스플레이 폰트는 `Design-token.md` 적용)
- GSAP + ScrollTrigger
- Swiper
- Lucide React
...
- 상품 목록·상세, 스토어, 메인 히어로, 배너에 사용하는 이미지는 Nintendo 공식 사이트(https://www.nintendo.com/kr/)의 실제 게임·제품·프로모션 이미지를 사용한다.
...
const products = [
    {
        id: 1,
        name: '슈퍼 마리오브라더스 원더',
        image: '/images/products/mario-wonder-01.webp',
    },
    {
        id: 2,
        name: '젤다의 전설 티어스 오브 더 킹덤',
        image: '/images/products/zelda-totk-01.webp',
    },
];
```
...
```text
잘못된 경로: public/images/products/mario-wonder-01.webp
올바른 경로: /images/products/mario-wonder-01.webp
```
...
| 폴더           | 용도                         | 예시                    |
| -------------- | ---------------------------- | ----------------------- |
| `products`     | 상품 목록·상세 이미지        | `mario-wonder-01.webp`        |
| `banners`      | 메인 히어로·프로모션 배너    | `hero-mario-01.webp`    |
...
```js
{
  ...
  image: "/images/products/mario-wonder-01.webp",
  images: [
    "/images/products/mario-wonder-01.webp",
    "/images/products/mario-wonder-02.webp"
  ],
  gameId: "mario-wonder",
  platform: "Nintendo Switch",
  productType: "SOFTWARE",
  formats: ["DOWNLOAD", "PACKAGE"],
  isNew: true
}
```
...
같은 방식으로 `productService.js`, `communityService.js`, `inquiryService.js`, `authService.js`를 분리한다.
...
- 디자인 시스템: [06 Design System](https://www.figma.com/design/e0m4kzzInBhdHR8Hrp5jPN/?node-id=450-10178)
- 정보구조: [정보구조 / FLOW SYNC](https://www.figma.com/design/e0m4kzzInBhdHR8Hrp5jPN/?node-id=382-7970)
- 작업 우선순위: [작업 페이지 선정](https://www.figma.com/design/e0m4kzzInBhdHR8Hrp5jPN/?node-id=2659-2601)
- 주력 페이지는 메인, 마리오·젤다·스플래툰, 역사다. 스토어·계정·커뮤니티·지원·결제는 연결되는 보조 화면으로 설계한다.
- IA의 관리자 항목은 운영 정의로 유지하고 이번 사용자용 프런트엔드의 관리자 페이지로 추가하지 않는다.
- 아래 URL과 React 이름은 Figma IA를 코드로 옮기기 위한 구현 명칭이다. Figma 원본 레이어 이름과 구분한다.
- Games는 콘텐츠 탐색, Store는 판매 상품 탐색으로 구분한다. 게임과 상품은 `gameId`로 연결하고 IP ↔ 게임 상세 ↔ 스토어 이동을 제공한다.
- News는 IA의 Home·GNB 연결을 위한 보조 목록이다. 계정 찾기는 기존 IA의 권장 항목이며 서버가 없으면 실제 메일·문자를 전송하지 않고 안내 상태를 제공한다.

| 경로 | 페이지 | 접근 권한 | 역할 |
|---|---|---|---|
| `/` | HomePage | 전체 | 메인·IP 및 게임 진입 |
| `/ip` | IpExplorePage | 전체 | IP·캐릭터 탐색 |
| `/ip/mario` | MarioPage | 전체 | 마리오 세계관·캐릭터·관련 게임 |
| `/ip/zelda` | ZeldaPage | 전체 | 젤다의 전설 세계관·캐릭터·관련 게임 |
| `/ip/splatoon` | SplatoonPage | 전체 | 스플래툰 플레이·무기·캐릭터·모드 |
| `/games` | GameExplorePage | 전체 | 검색·IP·장르·플레이 상황별 탐색 |
| `/games/results` | GameListPage | 전체 | 조건별 게임 목록·정렬 |
| `/games/:gameId` | GameDetailPage | 전체 | 게임 소개·영상·플레이 정보·구매 연결 |
| `/store` | StoreHomePage | 전체 | E-shop 진입 |
| `/store/products` | StoreProductListPage | 전체 | 소프트웨어·체험판·패스 상품 목록 |
| `/store/products/:productId` | StoreProductDetailPage | 전체 | 가격·옵션·체험 정보 |
...
| `/account/recovery` | AccountRecoveryPage | 비로그인 | 아이디·비밀번호 찾기 안내 |
| `/login` | LoginPage | 비로그인 | 기존 일반·카카오·테스트 로그인 |
| `/signup` | SignUpPage | 비로그인 | 회원가입 |
| `/auth/kakao/callback` | KakaoCallbackPage | 전체 | 카카오 인증 결과 |
| `/auth/error` | AuthErrorPage | 전체 | 인증 오류 |
| `/community` | CommunityFeedPage | 전체 | 게임·IP별 SNS형 피드 |
| `/community/:postId` | CommunityPostPage | 전체 | 게시물·댓글·반응 |
| `/community/write` | CommunityWritePage | 로그인 | 게시물 작성 |
| `/community/:postId/edit` | CommunityEditPage | 작성자 | 게시물 수정 |
| `/inquiries` | InquiryListPage | 로그인 | 내 고객문의 목록 |
| `/inquiries/write` | InquiryWritePage | 로그인 | 고객문의 작성 |
| `/inquiries/:inquiryId` | InquiryDetailPage | 작성자 | 고객문의 상세 |
| `/inquiries/:inquiryId/edit` | InquiryEditPage | 작성자 | 고객문의 수정 |
| `/mypage` | MyPage | 로그인 | 사용자 정보·활동·구매 내역 |
| `/mypage/profile` | ProfileEditPage | 로그인 | 내 정보 수정 |
...
- 메인 → IP 선택 → IP 상세의 캐릭터·세계관·플레이 콘텐츠 → 관련 게임·스토어로 연결한다.
- 게임 탐색 → 조건을 쿼리 문자열로 유지한 결과 목록 → 게임 상세 → 해당 판매 상품으로 연결한다.
- E-shop → 상품 목록 → 상품 상세 → 주문 확인 → 로그인 필요 시 인증 후 주문 확인 복귀 → 데모 결제 → 결과 확인으로 연결한다.
- 결제는 기존 프런트엔드 데모 범위를 유지한다. 성공·실패·취소 상태만 모의 처리하고 실제 결제 정보는 수집하지 않는다.
- 마이페이지에 사용자별 데모 구매 내역을 표시하며 기존 좋아요·최근 본 상품·프로필 기능을 유지한다.
- 커뮤니티는 기존 게시판 CRUD·권한·정렬·검색 규칙을 유지하고 SNS형 피드, 게임/IP 분류, 댓글·반응을 연결한다.
- 고객지원 랜딩은 전체 공개다. FAQ·문제 검색·상담·수리 안내에서 기존 `/inquiries`의 본인 문의 CRUD로 연결한다.

### 컴포넌트 명칭

| Figma 원본 또는 IA 역할 | React 컴포넌트 | 사용 위치 |
|---|---|---|
| Nav / Nav Type | NintendoHeader, NintendoNav | 공통 레이아웃 |
| Drop Down menu Type | NintendoMegaMenu | 공통 드롭다운·모바일 메뉴 |
| Arrow / NavIcons | NavigationArrow, NavigationIcon | 내비게이션 |
| 공통 Footer | NintendoFooter | 공통 레이아웃 |
| IP 탐색·캐릭터 소개 | IpCard, CharacterCard, IpStoreSection | 메인·IP 페이지 |
| 게임 탐색·목록 | GameFilter, GameCard, GameList | Games |
| E-shop 상품 | StoreProductCard, StoreProductList, StoreProductOptions | Store |
| 주문·결제 | OrderSummary, PaymentMethodSelector, CheckoutModal | Store |
| Card_History / Card_mobile | HistoryCard | 역사 |
| MobileNewscard | NewsCard | 메인·뉴스 |
| Social Card | CommunityPostCard | 커뮤니티 |
| 기존 게시판 목록·폼 | CommunityFeed, CommunityPostForm | 커뮤니티 |
| 최근활동 / 주문내역 / 남긴 리뷰 | RecentActivityList, OrderHistoryList, MyReviewList | 마이페이지 |
| FAQ·문제 검색 | FaqAccordion, SupportSearch | 고객지원 |

일반 Frame·con-box 이름은 그대로 코드 이름으로 사용하지 않고 실제 역할에 따라 매핑한다. 표의 IA 기반 이름은 구현용 제안이며 Figma에 동일한 컴포넌트가 등록되었다는 의미는 아니다.
...
Figma의 `Nav Type`과 `Drop Down menu Type`에 맞춰 주요 메뉴와 전체 메뉴를 나누어 배치한다. 모든 항목을 작은 내비게이션 바에 한 줄로 나열하지 않는다.
...
- 가장 최근에 본 상품부터 표시한다.
- 최대 10개까지만 저장한다.
- 상품 이미지, 상품명, 가격, 플랫폼·판매 형식, 신상품 여부를 표시한다.
...
좋아요 상품 또는 최근 본 상품이 없으면 설명과 스토어 이동 버튼을 표시한다.

---

## 14. 커뮤니티

SNS형 피드에서 게임·IP별 분류와 게시물 상세·댓글·반응을 제공한다. 기존 게시판 CRUD와 권한 규칙은 유지한다.
...
- 기본 게시글 최소 10개
- `useCommunityStore`와 localStorage 사용
...
│   │   ├── CommunityPostCard.css
│   │   ├── CommunityPostForm.jsx
│   │   └── CommunityPostForm.css
│   ├── common/
│   │   ├── AddressSearch.jsx
│   │   ├── AddressSearch.css
│   │   ├── Button.jsx
...
│   │   ├── NintendoNav.jsx
│   │   ├── NintendoNav.css
│   │   ├── NintendoMegaMenu.jsx
│   │   ├── NintendoMegaMenu.css
│   │   ├── NavigationArrow.jsx
│   │   ├── NavigationArrow.css
│   │   ├── NavigationIcon.jsx
│   │   ├── NavigationIcon.css
...
│   │   ├── RecentActivityList.css
│   │   ├── OrderHistoryList.jsx
│   │   ├── OrderHistoryList.css
│   │   ├── MyReviewList.jsx
│   │   └── MyReviewList.css
│   └── store/
│       ├── StoreProductCard.jsx
│       ├── StoreProductCard.css
│       ├── StoreProductList.jsx
│       ├── StoreProductList.css
│       ├── StoreProductOptions.jsx
│       ├── StoreProductOptions.css
│       ├── OrderSummary.jsx
│       ├── OrderSummary.css
...
│   │   ├── StoreHomePage.jsx
│   │   ├── StoreHomePage.css
│   │   ├── StoreProductListPage.jsx
│   │   ├── StoreProductListPage.css
│   │   ├── StoreProductDetailPage.jsx
│   │   ├── StoreProductDetailPage.css
│   │   ├── CartPage.jsx
│   │   ├── CartPage.css
│   │   ├── OrderReviewPage.jsx
│   │   ├── OrderReviewPage.css
...
│   │   ├── AccountRecoveryPage.css
│   │   ├── KakaoCallbackPage.jsx
│   │   ├── KakaoCallbackPage.css
│   │   ├── AuthErrorPage.jsx
│   │   └── AuthErrorPage.css
│   ├── community/
│   │   ├── CommunityFeedPage.jsx
│   │   ├── CommunityFeedPage.css
│   │   ├── CommunityPostPage.jsx
│   │   ├── CommunityPostPage.css
│   │   ├── CommunityWritePage.jsx
│   │   ├── CommunityWritePage.css
│   │   ├── CommunityEditPage.jsx
│   │   └── CommunityEditPage.css
│   ├── inquiry/
│   │   ├── InquiryListPage.jsx
│   │   ├── InquiryListPage.css
│   │   ├── InquiryWritePage.jsx
│   │   ├── InquiryWritePage.css
...
| `fonts.css`     | Pretendard 및 Figma 지정 폰트 연결                              |
...
```text
Nintendo Renewal — IP & Game Experience
```
...
- GSAP과 Swiper 사용 위치
- 상품·캐릭터 이미지 교체 방법
- Nintendo 공식 사이트 게임·제품·프로모션 이미지의 원본 페이지 URL과 사용 위치
...
- [ ] 메인·마리오·젤다·스플래툰·역사 페이지가 연결되었다.
- [ ] Games 탐색·목록·상세와 Store 상품 페이지가 구분되어 연결되었다.
- [ ] 주문 확인 → 인증 후 복귀 → 데모 결제 → 결과 확인이 동작한다.
- [ ] 커뮤니티 피드·상세·작성과 고객지원 → 내 문의가 연결되었다.
- [ ] 마이페이지에서 사용자별 데모 구매 내역을 확인할 수 있다.
- [ ] Figma 원본 스타일과 구현 기본값을 구분해 적용했다.
...
- [ ] 상품·스토어·히어로·배너에 Nintendo 공식 사이트 이미지를 사용하고 출처를 기록했다.
